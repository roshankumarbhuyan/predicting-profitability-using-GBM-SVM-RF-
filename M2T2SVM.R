...
#mydata <- read.csv("newproductattributes2017.2.csv")
names(mydata) <- c("Type","Number","price","Five","Four","Three","Two","One","Psr",
                   "Nsr","Rp","Bsr","Weight","Depth","Width","Height",
                   "Profit","Vol")
qplot(mydata$Vol, geom="histogram", bins= 30) +
ggplot(mydata, aes(Type,Vol,label = name))+geom_bar(check_overlap = TRUE)
#===================Preprocessing==========================================
summary(mydata)
head(mydata)
...
svm_tune <- train(Vol~., data = trainSet, method= "svmLinear", metric="RMSE")
svm_tune
...
#===========================test============================
svmpred = predict(svm_tune, newdata=readyData)
head(svmpred)
readyData$Vol <- svmpred
write.csv(readyData, file = "predictions.csv")
set.seed(849)
test_class_pred <- predict(svm_tune, testSet)
test_class_pred
postResample(pred =test_class_pred, obs = testSet$Vol)
